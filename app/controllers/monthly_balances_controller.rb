class MonthlyBalancesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @monthly_balance.save
        ...
        format.json { render action: 'show', status: :created, location: @monthly_balance }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to monthly_balances_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def monthly_balance_params
      params.require(:monthly_balance).permit(:account_id, :month, :date, :balance)
    end
end
